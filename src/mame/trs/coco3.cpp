...
static INPUT_PORTS_START( coco_rat_mouse )
	...
	PORT_START(RAT_MOUSE_LX_TAG)
		PORT_BIT( 0x03, 0x00,  IPT_MOUSE_X)
			...
			PORT_PLAYER(2)
	 		PORT_CONDITION(CTRL_SEL_RIGHT, 0x0f, EQUALS, coco_state::JOY_DEVICE_RAT_MOUSE)
			PORT_CHANGED_MEMBER(DEVICE_SELF, FUNC(coco_state::joystick_changed), 6)
	PORT_START(RAT_MOUSE_LY_TAG)
		PORT_BIT( 0x03, 0x00,  IPT_MOUSE_Y)
			...
			PORT_CHANGED_MEMBER(DEVICE_SELF, FUNC(coco_state::joystick_changed), 7)
...
void coco3_state::coco3(machine_config &config)
{
	...
	m_mux->address_changed_callback().set(FUNC(coco_state::mux_address_cb));
