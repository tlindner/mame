...
static INPUT_PORTS_START(ratmouse_r)
	...
	PORT_BIT(0x0fff, 0, IPT_MOUSE_X) PORT_SENSITIVITY(100) PORT_KEYDELTA(0) PORT_CHANGED_MEMBER(DEVICE_SELF, FUNC(quadencoder_device::changed), 0)
	PORT_CONDITION(":" CTRL_SEL_RIGHT, 0x0f, EQUALS, coco_state::JOY_DEVICE_RAT_MOUSE)

	PORT_START("y")
	PORT_BIT(0xf000, IP_ACTIVE_HIGH, IPT_UNUSED)
	PORT_BIT(0x0fff, 0, IPT_MOUSE_Y) PORT_SENSITIVITY(100) PORT_KEYDELTA(0) PORT_CHANGED_MEMBER(DEVICE_SELF, FUNC(quadencoder_device::changed), 0)
	PORT_CONDITION(":" CTRL_SEL_RIGHT, 0x0f, EQUALS, coco_state::JOY_DEVICE_RAT_MOUSE)
INPUT_PORTS_END
