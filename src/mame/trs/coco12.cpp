...
// // 	PORT_CONFSETTING(  0x04, "Tandy Hi-Res Interface" )                 PORT_CONDITION(CTRL_SEL_TAG, 0xf0, NOTEQUALS, 0x30)
// // 	PORT_CONFSETTING(  0x05, "CM3 Hi-Res Interface" )                   PORT_CONDITION(CTRL_SEL_TAG, 0xf0, NOTEQUALS, 0x30)
...
// // 	PORT_CONFSETTING(  0x40, "Tandy Hi-Res Interface" )                 PORT_CONDITION(CTRL_SEL_TAG, 0xf0, NOTEQUALS, 0x30)
// // 	PORT_CONFSETTING(  0x50, "CM3 Hi-Res Interface" )                   PORT_CONDITION(CTRL_SEL_TAG, 0xf0, NOTEQUALS, 0x30)
//
...
INPUT_PORTS_START( coco_joystick )
	PORT_START(JOYSTICK_RX_TAG)
		...
		PORT_PLAYER(1)
// 		PORT_CONDITION(CTRL_SEL_TAG, 0x0f, EQUALS, 0x01)
...
// 		PORT_CONDITION(CTRL_SEL_TAG, 0x0f, EQUALS, 0x01)
	PORT_START(JOYSTICK_LX_TAG)
		...
		PORT_CHANGED_MEMBER(DEVICE_SELF, FUNC(coco_state::joystick_changed), 4)
		PORT_SENSITIVITY(JOYSTICK_SENSITIVITY)
		PORT_KEYDELTA(JOYSTICK_DELTA)
		PORT_MINMAX(0,1023)
		PORT_CODE_DEC(KEYCODE_4_PAD)
		...
		PORT_CODE_INC(JOYCODE_X_RIGHT_SWITCH)
		PORT_PLAYER(2)
// 		PORT_CONDITION(CTRL_SEL_TAG, 0xf0, EQUALS, 0x10)
	PORT_START(JOYSTICK_LY_TAG)
		PORT_BIT( 0x3ff, 0x200,  IPT_AD_STICK_Y)
		PORT_NAME("Left Joystick Y")
		PORT_CHANGED_MEMBER(DEVICE_SELF, FUNC(coco_state::joystick_changed), 5)
		PORT_SENSITIVITY(JOYSTICK_SENSITIVITY)
		PORT_KEYDELTA(JOYSTICK_DELTA)
...
static INPUT_PORTS_START( cp400c2_keyboard )
	...
	PORT_BIT(0x10, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_NAME("PA1") PORT_CODE(KEYCODE_LALT)
	PORT_BIT(0x20, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_NAME("PA2") PORT_CODE(KEYCODE_RALT)
	PORT_BIT(0x40, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_NAME("PA3") PORT_CODE(KEYCODE_BACKSLASH)
	PORT_BIT(0x80, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_NAME("SHIFT") PORT_CODE(KEYCODE_LSHIFT) PORT_CODE(KEYCODE_RSHIFT) PORT_CHAR(UCHAR_SHIFT_1)
INPUT_PORTS_END
...
// 	m_pia_1->cb2_handler().set(FUNC(coco_state::pia1_cb2_w));
	...
	m_mux->zx_analog_callback().set(FUNC(coco_state::comparator_changed));
