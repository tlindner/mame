...
//============================================================
//
//  luaview.m - MacOS X Cocoa Lua debug window handling
//
//============================================================
...
- (BOOL)selectSubviewForSpace:(address_space *)space {
	...
	{
		debug_view_lua_source const *const source = downcast<debug_view_lua_source const *>(ptr.get());
...
- (NSString *)expression {
	return [NSString stringWithUTF8String:downcast<debug_view_lua *>(view)->expression()];
}


- (void)setExpression:(NSString *)exp {
	downcast<debug_view_lua *>(view)->set_expression([exp UTF8String]);
}


- (debug_view_lua_source const *)source {
	return downcast<debug_view_lua_source const *>(view->source());
}
...
- (IBAction)loadScript:(id)sender {
	...
	[panel beginWithCompletionHandler: ^(NSInteger result) {
		if (result == NSModalResponseOK) {
			NSURL *theScript = [[panel URLs] objectAtIndex:0];
...
// 	downcast<debug_view_lua *>(view)->set_data_format(debug_view_lua::data_format([sender tag]));
}


- (IBAction)showPhysicalAddresses:(id)sender {
// 	downcast<debug_view_lua *>(view)->set_physical([sender tag]);
}

- (IBAction)showAddressRadix:(id)sender {
// 	downcast<debug_view_lua *>(view)->set_address_radix([sender tag]);
}

- (IBAction)showReverseView:(id)sender {
// 	downcast<debug_view_lua *>(view)->set_reverse([sender tag]);
}


- (IBAction)showReverseViewToggle:(id)sender {
// 	downcast<debug_view_lua *>(view)->set_reverse(!downcast<debug_view_lua *>(view)->reverse());
}


- (IBAction)changeBytesPerLine:(id)sender {
// 	debug_view_lua *const memView = downcast<debug_view_lua *>(view);
...
- (void)saveConfigurationToNode:(util::xml::data_node *)node {
	[super saveConfigurationToNode:node];
 	debug_view_lua *const luaView = downcast<debug_view_lua *>(view);
 	node->set_attribute(osd::debugger::ATTR_WINDOW_LUA_SCRIPT_PATH, luaView->script());
 	node->set_attribute_int(osd::debugger::ATTR_WINDOW_LUA_RUNNING, luaView->running() ? 1 : 0);
...
- (void)restoreConfigurationFromNode:(util::xml::data_node const *)node {
	[super restoreConfigurationFromNode:node];
 	debug_view_lua *const luaView = downcast<debug_view_lua *>(view);
 	luaView->set_script(node->get_attribute_string(osd::debugger::ATTR_WINDOW_LUA_SCRIPT_PATH, ""));
 	luaView->set_running(node->get_attribute_int(osd::debugger::ATTR_WINDOW_LUA_RUNNING, luaView->running() ? 1 : 0));
...
// 	memView->set_data_format(debug_view_lua::data_format(node->get_attribute_int(osd::debugger::ATTR_WINDOW_MEMORY_DATA_FORMAT, int(memView->get_data_format()))));
...
- (void)insertActionItemsInMenu:(NSMenu *)menu atIndex:(NSInteger)index {
	NSMenuItem  *loadScript = [menu insertItemWithTitle:@"Load Script…"
												 action:@selector(loadScript:)
										  keyEquivalent:@"l"
												atIndex:index++];
	[loadScript setTarget:self];

	NSMenuItem  *reloadScript = [menu insertItemWithTitle:@"Reload Script"
												 action:@selector(reloadScript:)
										  keyEquivalent:@"r"
												atIndex:index++];
	[reloadScript setTarget:self];

	NSMenuItem  *stopScript = [menu insertItemWithTitle:@"Stop Script"
												 action:@selector(stopScript:)
										  keyEquivalent:@"s"
												atIndex:index++];
	[stopScript setTarget:self];
...
// 	[chunkItem1 setTag:int(debug_view_lua::data_format::HEX_8BIT)];
//
...
// 	[chunkItem2 setTag:int(debug_view_lua::data_format::HEX_16BIT)];
//
...
// 	[chunkItem4 setTag:int(debug_view_lua::data_format::HEX_32BIT)];
//
...
// 	[chunkItem8 setTag:int(debug_view_lua::data_format::HEX_64BIT)];
//
...
// 	[chunkItem12 setTag:int(debug_view_lua::data_format::OCTAL_8BIT)];
//
...
// 	[chunkItem13 setTag:int(debug_view_lua::data_format::OCTAL_16BIT)];
//
...
// 	[chunkItem14 setTag:int(debug_view_lua::data_format::OCTAL_32BIT)];
//
...
// 	[chunkItem15 setTag:int(debug_view_lua::data_format::OCTAL_64BIT)];
//
...
// 	[chunkItem9 setTag:int(debug_view_lua::data_format::FLOAT_32BIT)];
//
...
// 	[chunkItem10 setTag:int(debug_view_lua::data_format::FLOAT_64BIT)];
//
...
// 	[chunkItem11 setTag:int(debug_view_lua::data_format::FLOAT_80BIT)];
//
// 	[menu insertItem:[NSMenuItem separatorItem] atIndex:index++];
//
