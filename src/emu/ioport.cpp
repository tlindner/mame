...
void ioport_manager::dump_port_tree() const
{
	// Guard against calling before ports are initialized
    if (machine().phase() < machine_phase::RESET)
    {
        osd_printf_warning("I/O Port dump skipped: Machine is still initializing.\n");
        return;
    }

	auto print_condition = [](const char *prefix, ioport_condition const &cond) {
		if (cond.none())
			return;

		static char const *const s_condition_names[] = {
				"ALWAYS", "EQUALS", "NOTEQUALS", "GREATERTHAN",
				"NOTGREATERTHAN", "LESSTHAN", "NOTLESSTHAN" };
		unsigned const cond_index = unsigned(cond.condition());

		osd_printf_info("%s[Condition] %s Depends on Tag: '%s' | Mask: 0x%08X | Value: 0x%08X\n",
			prefix,
			(cond_index < std::size(s_condition_names)) ? s_condition_names[cond_index] : "?",
			cond.tag() ? cond.tag() : "None",
			cond.mask(),
			cond.value());
	};

	auto print_seq = [this](const char *prefix, const char *label, input_seq const &seq) {
		if (seq.empty())
			return;

		std::string const tokens = machine().input().seq_to_tokens(seq);
		osd_printf_info("%s[Seq] %-9s : %s\n", prefix, label, tokens.c_str());
	};

	auto type_class_name = [](ioport_type_class cls) -> char const * {
		switch (cls)
		{
		case INPUT_CLASS_INTERNAL:   return "INTERNAL";
	...
	osd_printf_info("=== I/O PORT TREE DUMP ===\n");

	if (m_portlist.empty())
	{
		osd_printf_info("  [No ports registered]\n");
		return;
	}

	bool const live = safe_to_read();

	for (auto const &[tag, port] : m_portlist)
	{
		osd_printf_info("Port Tag: %s (Device: %s) | Active Mask: 0x%08X | Modcount: %d\n",
			port->tag(),
			port->device().tag(),
			port->active(),
			port->modcount());
		...
		if (live)
		{
			ioport_port_live const &plive = port->live();
			osd_printf_info("  | [Live] Default: 0x%08X | Digital: 0x%08X | Output: 0x%08X | Analog Fields: %u | Read Fields: %u | Write Fields: %u\n",
				plive.defvalue,
				plive.digital,
				plive.outputvalue,
				unsigned(plive.analoglist.size()),
				unsigned(plive.readlist.size()),
				unsigned(plive.writelist.size()));
		}

		for (ioport_field const &field : port->fields())
		{
			ioport_type_class const cls = field.type_class();

			osd_printf_info("  |- Field: %-20s | Mask: 0x%08X | Default: 0x%08X | Type: %d | Class: %s | Player: %d%s\n",
				field.name().c_str(),
				field.mask(),
				field.defvalue(),
				field.type(),
			...
			{
				std::string flags;
				if (field.cocktail())          flags += " Cocktail";
				if (field.toggle())            flags += " Toggle";
				if (field.impulse())           flags += " Impulse(" + std::to_string(field.impulse()) + ")";
				if (field.has_dynamic_read())  flags += " DynamicRead";
				if (field.has_dynamic_write()) flags += " DynamicWrite";
				if (!flags.empty())
					osd_printf_info("  |   | [Flags]%s\n", flags.c_str());
			}

			// Field-level condition (most common, e.g., DIP switch conditional visibility)
			print_condition("  |   | ", field.condition());

			// Input sequences bound to this field
			print_seq("  |   | ", "Standard", field.seq(SEQ_TYPE_STANDARD));
			print_seq("  |   | ", "Increment", field.seq(SEQ_TYPE_INCREMENT));
			print_seq("  |   | ", "Decrement", field.seq(SEQ_TYPE_DECREMENT));

			// Analog-specific configuration
			if (field.is_analog())
			{
				osd_printf_info("  |   | [Analog] Min: 0x%08X | Max: 0x%08X | Sensitivity: %d | Delta: %d | CenterDelta: %d%s%s%s\n",
					field.minval(),
					field.maxval(),
					field.sensitivity(),
					field.delta(),
					field.centerdelta(),
					field.analog_reverse() ? " | Reverse" : "",
					field.analog_wraps() ? " | Wraps" : "",
					field.analog_invert() ? " | Invert" : "");

				if (field.analog_reset())
					osd_printf_info("  |   |   | Reset-to-default each frame (relative axis)\n");

				if (field.remap_table())
					osd_printf_info("  |   |   | Remap table present\n");

				if (field.full_turn_count())
					osd_printf_info("  |   |   | Full turn count: %u\n", field.full_turn_count());

				if (field.crosshair_axis() != CROSSHAIR_AXIS_NONE)
					osd_printf_info("  |   |   | Crosshair Axis: %s | Scale: %f | Offset: %f | AltAxis: %f\n",
						field.crosshair_axis() == CROSSHAIR_AXIS_X ? "X" : "Y",
						field.crosshair_scale(),
						field.crosshair_offset(),
						field.crosshair_altaxis());

				if (live)
				{
					ioport_field_live const &flive = field.live();
					if (flive.analog)
					{
						osd_printf_info("  |   |   | [Live] Sensitivity: %d | Reverse: %s | Delta: %d | CenterDelta: %d\n",
							flive.analog->sensitivity(),
							flive.analog->reverse() ? "Yes" : "No",
							flive.analog->delta(),
							flive.analog->centerdelta());
					}
				}
			}

			// Digital joystick way restriction (2/4/8-way etc.)
			if (field.is_digital_joystick() && field.way())
				osd_printf_info("  |   | [Joystick] Way: %u\n", field.way());

			// Keyboard character mappings
			if (cls == INPUT_CLASS_KEYBOARD)
			{
				for (int shift = 0; shift < 2; shift++)
				{
					std::vector<char32_t> const codes = field.keyboard_codes(shift);
					if (!codes.empty())
					{
						std::string const keyname = field.key_name(shift);
						osd_printf_info("  |   | [Key %s] %s\n",
							shift ? "Shift" : "Base",
							keyname.empty() ? "(unnamed)" : keyname.c_str());
					}
				}
			}

			// Live state (current value, toggle, lockout) when available
			if (live)
			{
				ioport_field_live const &flive = field.live();
				...
				std::string extra;
				if (!flive.name.empty())
					extra = " | Override Name: " + flive.name;

				osd_printf_info("  |   | [Live] Value: 0x%08X | Last: %s | Toggle: %s | Lockout: %s%s\n",
					flive.value,
					flive.last ? "Yes" : "No",
					flive.toggle ? "Yes" : "No",
					flive.lockout ? "Yes" : "No",
					extra.c_str());
			}

			// Print settings for DIP switches / configs
			for (ioport_setting const &setting : field.settings())
			{
				osd_printf_info("  |   |- Setting: %-16s | Value: 0x%08X %s\n",
					setting.name(),
					setting.value(),
					setting.enabled() ? "" : "(Disabled)");

				// Setting-level condition
				print_condition("  |   |   | ", setting.condition());
			}

			// Print DIP switch location bindings
			for (ioport_diplocation const &diploc : field.diplocations())
			{
				osd_printf_info("  |   |- DIP Switch: %s Line %d (Invert: %s)\n",
					diploc.name(),
					diploc.number(),
					diploc.inverted() ? "Yes" : "No");
			}
		}
	}

	osd_printf_info("=== END OF PORT TREE DUMP ===\n");
}
