...
/*********************************************************************
    ...
    Debug window controlled by Lua script.
...
public:
	...
	debug_view_lua_source(std::string &&name, memory_region &region);
	debug_view_lua_source(std::string &&name, void *base, int element_size, int num_elements, int num_blocks, int block_stride);
...
// 	static bool is_valid_format(data_format format) { return int(format) >= 0 && int(format) < std::size(s_memory_pos_table) && get_posdata(format).m_bytes != 0; }
	...
	const char *script() const { return m_script_path.c_str(); }
