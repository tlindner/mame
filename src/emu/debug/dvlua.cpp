...
// const debug_view_lua::memory_view_pos debug_view_lua::s_memory_pos_table[16] =
// {
...
debug_view_lua_source::debug_view_lua_source(std::string &&name, memory_region &region)
...
debug_view_lua_source::debug_view_lua_source(std::string &&name, void *base, int element_size, int num_elements, int num_blocks, int block_stride)
	: debug_view_source(std::move(name))
	, m_space(nullptr)
	, m_memintf(nullptr)
	, m_base(base)
...
// 	m_section[1].m_pos = m_section[0].m_pos + m_section[0].m_width;
    ...
    m_lua.get()->sol().set_function("view_clear", &debug_view_lua::clear, this);
...
void debug_view_lua::clear()
{
	begin_update();
	std::fill(m_viewbuffer.begin(), m_viewbuffer.end(), ' ');
	view_set_xy(0, 0);
	end_update();
}
...
//  sources for a lua view
//-------------------------------------------------

void debug_view_lua::enumerate_sources()
{
		...
		for (int spacenum = 0; spacenum < memintf.max_space_count(); ++spacenum)
		{
			if (memintf.has_space(spacenum))
			{
				...
				m_source_list.emplace_back(
						std::make_unique<debug_view_lua_source>(
	...
	for (int itemnum = 0; itemnum < machine().save().registration_count(); itemnum++)
	{
		...
		if (strncmp(name.c_str(), "timer/", 6))
			m_source_list.emplace_back(std::make_unique<debug_view_lua_source>(std::move(name), base, valsize, valcount, blockcount, stride));
	}
	std::sort(
...
// 		const debug_view_lua_source &source = downcast<const debug_view_lua_source &>(*m_source);
...
// void debug_view_lua::generate_row(debug_view_char *destmin, debug_view_char *destmax, debug_view_char *destrow, offs_t address)
// {
// 	// get positional data
...
//  structure of the lua view
//-------------------------------------------------

// void debug_view_lua::recompute()
// {
// 	const debug_view_lua_source &source = downcast<const debug_view_lua_source &>(*m_source);
//
...
// bool debug_view_lua::needs_recompute()
// {
...
// debug_view_lua::cursor_pos debug_view_lua::get_cursor_pos(const debug_view_xy& cursor)
// {
...
// void debug_view_lua::set_cursor_pos(cursor_pos pos)
// {
// 	const memory_view_pos &posdata = get_posdata(m_data_format);
//
...
//-------------------------------------------------
//  read - generic lua view data reader
//-------------------------------------------------

// bool debug_view_lua::read(u8 size, offs_t offs, u64 &data)
// {
// 	const debug_view_lua_source &source = downcast<const debug_view_lua_source &>(*m_source);
//
...
// bool debug_view_lua::read(u8 size, offs_t offs, extFloat80_t &data)
// {
// 	u64 t;
...
//-------------------------------------------------
//  read_chunk - lua view data reader helper
//-------------------------------------------------

// bool debug_view_lua::read_chunk(offs_t address, int chunknum, u64 &chunkdata)
// {
// 	const debug_view_lua_source &source = downcast<const debug_view_lua_source &>(*m_source);
// 	if (source.m_space) {
...
//-------------------------------------------------
//  write - generic lua view data writer
//-------------------------------------------------

// void debug_view_lua::write(u8 size, offs_t offs, u64 data)
// {
// 	const debug_view_lua_source &source = downcast<const debug_view_lua_source &>(*m_source);
//
// 	// if no raw data, just use the standard debug routines
...
// bool debug_view_lua::write_digit(offs_t offs, u8 pos, u8 digit)
// {
// 	const debug_view_lua_source &source = downcast<const debug_view_lua_source &>(*m_source);
...
void debug_view_lua::set_expression(const std::string &expression)
{
	begin_update();
...
//-------------------------------------------------
//  set_script - set the path string
//  to find the script
//-------------------------------------------------
...
// void debug_view_lua::set_chunks_per_row(u32 rowchunks)
// {
// 	if (rowchunks < 1)
// 		return;
//
...
// void debug_view_lua::set_data_format(data_format format)
// {
// 	cursor_pos pos;
//
// 	// should never be
...
//-------------------------------------------------
//  set_reverse - specify true if the lua view
//  is displayed reverse
//-------------------------------------------------

// void debug_view_lua::set_reverse(bool reverse)
// {
// 	cursor_pos pos = begin_update_and_get_cursor_pos();
...
//-------------------------------------------------
//  set_ascii - specify true if the lua view
//  should display an ASCII representation
//-------------------------------------------------

// void debug_view_lua::set_ascii(bool ascii)
// {
// 	cursor_pos pos = begin_update_and_get_cursor_pos();
// 	m_ascii_view = ascii;
// 	m_recompute = m_update_pending = true;
...
//-------------------------------------------------
//  set_physical - specify true if the lua view
//  should display physical addresses versus
//  logical addresses
//-------------------------------------------------

// void debug_view_lua::set_physical(bool physical)
// {
// 	cursor_pos pos = begin_update_and_get_cursor_pos();
...
//-------------------------------------------------
//  set_address_radix - specify whether the lua
//  view should display addresses in base 8, base
//  10 or base 16
//-------------------------------------------------

// void debug_view_lua::set_address_radix(int radix)
// {
